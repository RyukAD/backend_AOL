...
function checkFileType(file, cb){
  ...
  if (mimetype && extname){
    ...
    return cb("Image file type or Image Mime type are not valid")
  }
}
  ...
  // }
    ...
    if (user) {
      ...
      const msg = {
        ...
        subject: 'Activation link to activate your account',
        html: `<h1>Click the link below to activate your account</h1>
              <p>${process.env.CLIENT_URL}/api/users/volunteer/confirmation/${req.body.email}/${token.token}</p>
              `
      }
...
router.get('/volunteer/confirmation/:email/:token', (req, res) => {
  ...
  Token.findOne({token: req.params.token}, (err, token) => {
    ...
    //if token found check valid user
      ...
      Volunteer.findOne({ email: req.params.email}, (err, user) => {
        //no user found
        if (!user){
  ...
  Teacher.findOne({ email: req.body.email }).then(user => {
    if (user) {
      ...
      const newUser = new Teacher({
        ...
        alternateNumber: req.body.alternateNumber,
        teacherIdImage: {
          data: fs.readFileSync(path.join('../backend/public/uploads/' + req.file.filename)),
      ...
      const msg = {
        ...
        subject: 'Activation link to activate your account',
        html: `<h1>Click the link below to activate your account</h1>
              <p>${process.env.CLIENT_URL}/api/users/teacher/confirmation/${req.body.email}/${token.token}</p>
              `
      }
      sgMail.send(msg).then(() => {
        return res.status(200).send({msg: 'Email sent Successfully'})
...
router.get('/teacher/confirmation/:email/:token', (req, res) => {
  ...
  Token.findOne({token: req.params.token}, (err, token) => {
    ...
    //if token found check valid user
      ...
      Teacher.findOne({ email: req.params.email}, (err, user) => {
        //no user found
        if (!user){
          return res.status(401).send({msg: "We're not able to find a valid user for this verification link"})
        }
